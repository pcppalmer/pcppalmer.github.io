    ...
    <div className="shell">
      ...
      <main id="main">
        ...
        <section id="projects" aria-labelledby="projects-title">
          <div className="section-heading"><div><p className="eyebrow">A FEW THINGS I'VE BUILT</p><h2 id="projects-title">Projects<span className="muted"> / 02</span></h2></div><div className="filters" role="group" aria-label="Filter projects">{['All projects', 'Web', 'Games'].map(label => <button key={label} aria-pressed={filter === label} onClick={() => setFilter(label)}>{label}</button>)}</div></div>
          <div className="projects-grid">
            ...
            {filter !== 'Web' && <article className="project"><a className="project-art goblin-art" href="https://patrickpalmer.xyz/goblin-order-rush/" aria-label="Play Goblin Order Rush"><div className="art-top"><Arrow /></div><div className="game-art" aria-hidden="true"><div className="spark spark-one">✦</div><div className="spark spark-two">✧</div><strong>GOBLIN<br /><em>ORDER RUSH</em></strong><code><span>SELECT</span> * <span>FROM</span> inventory;</code></div></a><div className="project-info"><div className="project-title"><h3><a href="https://patrickpalmer.xyz/goblin-order-rush/">Goblin Order Rush</a></h3><span className="tag">BROWSER GAME</span></div><p>Run a goblin shop. Learn a little SQL.<br />Turn queries into gold, one order at a time.</p><div className="project-links"><a href="https://patrickpalmer.xyz/goblin-order-rush/">Play the game <Arrow /></a><a href={`${github}/goblin-order-rush`}>Source code <Arrow /></a></div></div></article>}
          </div>
        </section>
      </main>
